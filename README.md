...
```sh
cargo run -- ui --demo --cache-dir /tmp/tokctl-demo --overwrite
```

Use `tokctl demo --cache-dir /tmp/tokctl-demo --overwrite` to seed the same synthetic cache without opening the UI. The demo data is generated by `src/demo.rs`. It uses fake repo names and `/demo/...` paths.

---

## Status / limitations

`tokctl` is a local CLI for personal inspection of Claude, Codex, and Cursor usage logs. It is not a billing source of truth.

- Prices come from a static table in `src/pricing.rs`; unknown Claude/Codex models are shown as `$0` with a warning.
- Cursor costs come from local CSV exports when present.
- Repo attribution depends on paths in local logs and nearest `.git` directories.
- `--demo` and the README screenshot use synthetic data only. They do not read personal logs.
